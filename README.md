...
│   └── DutyManager/          # C3 跟场任务【B·已完成：8 action 已实装】
└── project.config.json
```

**进度**：AuthManager、DutyManager（B·跟场线）已完成；待创建的业务云函数：`ScheduleManager`（A）、`DashboardManager`/`ArchiveManager`/`TimerChecker`（C）、`CalendarManager`/`TeamManager`（收尾）。
